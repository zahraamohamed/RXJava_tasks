...
object RXJavaTasks {
    ...
    fun task1(): Observable<String> {
        return Observable.just("ABCDEFGHIJKLMNOPQRSTUVWXYZ".split(""))
    ...
    fun task3(): Observable<String> {
        ...
        return firstObservable.mergeWith(secondObservable.map {
            it.toString()
        }
        ).zipWith(Observable.interval(300, TimeUnit.MILLISECONDS), { item, _ -> item })
    }
    ...
    fun task5(): Observable<String> {
        ...
        val secondObservable = Observable.range(1, 5)
            .zipWith(Observable.interval(300, TimeUnit.MILLISECONDS), { item, _ -> item })
        return Observable.zip(firstObservable, secondObservable, { char, number -> "$char$number" })
    }
}
